...
If the air changes, you may be on the right track.`,
    ...
    checkin: {
      prompt: 'Enter the place name shown at this checkpoint.',
      placeholder: '',
      validator: value => {
        const compact = normalize(value).replace(/[^a-z]/g, '');
        return compact === 'urbanfarm' || compact === 'rm';
      },
      expected: 'Urban Farm'
    },
...
Is this choice truly better, or just greener-looking?`,
    ...
    checkin: {
      prompt: 'Enter the shop name shown at this checkpoint.',
      placeholder: '',
      validator: value => {
        const v = normalize(value).replace(/[^a-z0-9]/g, ' ').replace(/\s+/g, ' ').trim();
        return ['the green collective sg', 'the green collective', 'green collective', 'green collective sg', 'collective', 'green'].includes(v);
      },
      expected: 'The Green Collective SG'
    },
...
Should we ride, push, walk beside it… or rethink it?`,
    ...
    checkin: {
      prompt: 'Enter the number shown on the cycling route rule sign.',
      placeholder: '',
...
Find the doorway to a different kind of stay.`,
    ...
    checkin: {
      prompt: 'Enter the logo word shown at this checkpoint.',
      placeholder: '',
...
function renderAccessorySvg(upgrade) {
  ...
  const id = upgrade.id || '';
  const wrap = (inner, cls='') => `<g class="upgrade-svg ${escapeHtml(slot)} ${escapeHtml(id)} ${cls}">${inner}</g>`;
  if (id === 'gardenHat') return wrap(`
    <ellipse cx="132" cy="42" rx="42" ry="12" fill="#eacb75" stroke="#b68338" stroke-width="3"/>
    <path d="M104 42 Q132 6 160 42 L160 52 Q132 58 104 52 Z" fill="#f2d790" stroke="#b68338" stroke-width="3"/>
    <path d="M112 45 Q132 24 152 45" fill="none" stroke="#7cc26a" stroke-width="6" stroke-linecap="round"/>
    <circle cx="145" cy="25" r="5" fill="#8fd46e"/>
  `);
  if (id === 'flowerHat') return wrap(`
    <ellipse cx="132" cy="45" rx="38" ry="10" fill="#e7d2a1" stroke="#be9a5d" stroke-width="3"/>
    <path d="M106 43 Q132 16 158 43 L158 50 Q132 54 106 50 Z" fill="#f3e1b8" stroke="#be9a5d" stroke-width="3"/>
    <g transform="translate(99 18)">
      <circle cx="15" cy="16" r="6" fill="#ff8fab"/><circle cx="28" cy="9" r="6" fill="#ffe066"/><circle cx="42" cy="16" r="6" fill="#8fd3ff"/><circle cx="56" cy="11" r="6" fill="#ffa94d"/>
      <circle cx="15" cy="16" r="2.2" fill="#fff"/><circle cx="28" cy="9" r="2.2" fill="#fff"/><circle cx="42" cy="16" r="2.2" fill="#fff"/><circle cx="56" cy="11" r="2.2" fill="#fff"/>
      <path d="M8 22 Q30 28 62 22" fill="none" stroke="#79b35a" stroke-width="4" stroke-linecap="round"/>
    </g>
  `);
  if (id === 'toteBadge') return wrap(`
    <path d="M118 146 h24 a4 4 0 0 1 4 4 v24 a4 4 0 0 1 -4 4 h-24 a4 4 0 0 1 -4 -4 v-24 a4 4 0 0 1 4 -4 z" fill="#6fd2a5" stroke="#0b7f8a" stroke-width="3"/>
    <path d="M121 150 q9 -12 18 0" fill="none" stroke="#0b7f8a" stroke-width="3" stroke-linecap="round"/>
  `);
  if (id === 'judgeBadge') return wrap(`
    <path d="M132 161 l8 4 -2 8 6 6 -6 6 2 8 -8 4 -8 -4 -8 4 2 -8 -6 -6 6 -6 -2 -8 8 -4 8 -4z" fill="#ffd166" stroke="#e09f1f" stroke-width="3"/>
    <path d="M125 176 l5 5 10 -12" fill="none" stroke="#0b7f8a" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
  `);
  if (id === 'tinyWheels') return wrap(`
    <circle cx="110" cy="238" r="10" fill="#173846"/><circle cx="154" cy="238" r="10" fill="#173846"/>
    <path d="M112 220 h34 l10 16 h-54 z" fill="#74c0fc" stroke="#087f8c" stroke-width="3" stroke-linejoin="round"/>
  `);
  if (id === 'greenSneakers') return wrap(`
    <path d="M102 233 q10 0 16 8 v8 h-22 q0 -10 6 -16z" fill="#53c26f" stroke="#2b8a3e" stroke-width="3"/>
    <path d="M146 233 q10 0 16 8 v8 h-22 q0 -10 6 -16z" fill="#53c26f" stroke="#2b8a3e" stroke-width="3"/>
    <path d="M98 245 h22 M142 245 h22" stroke="#fff" stroke-width="3" stroke-linecap="round"/>
  `);
  if (id === 'sgBruneiScarf') return wrap(`
    <path d="M101 138 q30 14 61 0" fill="none" stroke="#ef233c" stroke-width="8" stroke-linecap="round"/>
    <path d="M101 145 q30 14 61 0" fill="none" stroke="#ffd166" stroke-width="8" stroke-linecap="round"/>
    <rect x="145" y="142" width="8" height="28" rx="4" fill="#ef233c"/>
    <rect x="154" y="145" width="8" height="24" rx="4" fill="#ffd166"/>
  `);
  if (id === 'cityBandana') return wrap(`
    <path d="M103 138 h58 l-10 18 -19 -9 -19 9 z" fill="#2fbf71" stroke="#1a8b4e" stroke-width="3" stroke-linejoin="round"/>
    <path d="M132 146 l6 8 h-12 z" fill="#fff"/>
  `);
  if (id === 'reusableBottle') return wrap(`
    <rect x="166" y="145" width="16" height="38" rx="7" fill="#74c0fc" stroke="#087f8c" stroke-width="3"/>
    <rect x="170" y="138" width="8" height="9" rx="3" fill="#087f8c"/>
  `);
  if (id === 'snackPouch') return wrap(`
    <path d="M162 148 h24 v30 q0 6 -6 6 h-12 q-6 0 -6 -6 z" fill="#ffd166" stroke="#ff922b" stroke-width="3"/>
    <path d="M168 160 l3 3 6 -7" fill="none" stroke="#8f5b37" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
  `);
  return '';
}
...
function renderUpgrade() {
  ...
  app.innerHTML = `
    <section class="card">
      ...
      <h3>Choose one:</h3>
      <div class="grid two">
        ${cfg.options.map(opt => `<button type="button" class="option-card upgrade-choice ${chosen === opt.id ? 'selected' : ''}" data-upgrade="${escapeHtml(opt.id)}"><div class="upgrade-swatch ${escapeHtml(opt.id)}"></div><div><strong>${escapeHtml(opt.name)}</strong><span>Your otter will keep this upgrade until the end.</span></div></button>`).join('')}
      </div>
