...
const CONFIG = {
  ...
  DEVICE_ID_KEY: 'ecoXpFunanDeviceId_v1',
  MAX_BUDDIES_PER_TEAM: 2,
...
function getDeviceId() {
  ...
  if (!id) {
    const randomPart = (crypto && crypto.getRandomValues)
...
function isBuddy() { return state.role === 'buddy'; }
function canInput() { return !isBuddy(); }

function roleLabel() {
  if (isCaptain()) return 'Captain / Input Device';
  if (isBuddy()) return 'Buddy Viewer';
...
function loadState() {
  try {
    ...
    return raw ? { ...defaultState(), ...JSON.parse(raw), deviceId: getDeviceId() } : defaultState();
  } catch (err) {
...
function setScreen(screen, opts = {}) {
  ...
  if (isCaptain() && state.started && !['splash', 'login', 'role', 'device', 'rules', 'briefing'].includes(screen)) {
    backend('saveProgress', serializeProgress()).catch(err => console.warn('Screen progress sync failed', err));
  }
...
function mockBackend(action, payload) {
  ...
  if (action === 'joinTeam') {
    return { ok: true, mock: true, role: payload.role, teamState: null, buddyCount: payload.role === 'buddy' ? 1 : 0, maxBuddies: CONFIG.MAX_BUDDIES_PER_TEAM };
  }
  if (action === 'getTeamState') {
    return { ok: true, mock: true, teamState: { group: state.group, teamName: state.teamName, progress: serializeProgress() }, buddyCount: isBuddy() ? 1 : 0, maxBuddies: CONFIG.MAX_BUDDIES_PER_TEAM };
  }
...
function render() {
  ...
  if (isBuddy() && state.currentScreen !== 'login' && state.currentScreen !== 'role') return renderBuddyView();
...
function getSafeResumeScreen() {
  ...
  if (!state.roleJoined || !state.role) return 'role';
...
function renderRoleSelect() {
  ...
  app.innerHTML = `
    <section class="card">
      <span class="kicker">Team Device Role</span>
      <h2>Choose how this device will join.</h2>
      <p><strong>${escapeHtml(state.group)} — ${escapeHtml(state.teamName)}</strong></p>
      <div class="section-card">
        <h3>Captain / Input Device</h3>
        <p>Only <strong>one</strong> captain device can submit answers, upload photos, choose upgrades, and move the team forward.</p>
        <button class="primary-btn" id="joinCaptain">Join as Captain / Input Device</button>
      </div>
      <div class="section-card">
        <h3>Buddy Viewer</h3>
        <p>Up to <strong>two</strong> buddy devices can follow the same team, read instructions, view options, and manually refresh progress, but cannot submit.</p>
        <button class="secondary-btn" id="joinBuddy">Join as Buddy Viewer</button>
      </div>
      <p><small>Team members do not need to join at the same time. Buddies can join later and press Refresh to follow the captain’s progress.</small></p>
      <div class="actions"><button class="ghost-btn" id="backLogin">Back</button></div>
    </section>`;
  $('#joinCaptain').addEventListener('click', () => joinTeamRole('captain'));
  $('#joinBuddy').addEventListener('click', () => joinTeamRole('buddy'));
  $('#backLogin').addEventListener('click', () => setScreen('login'));
}

async function joinTeamRole(role) {
  showLoading(role === 'captain' ? 'Registering captain device…' : 'Joining as buddy viewer…');
  try {
    const result = await backend('joinTeam', {
      group: state.group,
      teamName: state.teamName,
      route: state.route,
      role,
    ...
    const keep = { role, roleJoined: true, deviceId: state.deviceId || getDeviceId() };
    if (result.teamState && result.teamState.progress) {
      applyTeamProgress(result.teamState.progress, keep);
    } else {
      state.role = role;
      state.roleJoined = true;
      state.deviceId = keep.deviceId;
    }
    state.group = state.group || result.group;
    state.teamName = state.teamName || result.teamName;
    state.route = state.route?.length ? state.route : ROUTES[state.group];
    if (role === 'captain') {
      state.currentScreen = state.started ? getSafeResumeScreen() : 'device';
      saveState();
      setScreen(state.currentScreen);
    } else {
      state.currentScreen = result.teamState?.progress?.currentScreen || 'checkpoint';
      state.started = Boolean(result.teamState?.progress?.started || result.teamState?.progress);
      state.buddyLastRefresh = new Date().toISOString();
      saveState();
      renderBuddyView();
    }
  } catch (err) {
    ...
    app.innerHTML = `
      <section class="card">
        <span class="kicker">Role unavailable</span>
        <h2>B.O.B. could not assign this role.</h2>
        <p>${escapeHtml(err.message || err)}</p>
        <p><small>There can be only one captain/input device and up to two buddy viewers per team.</small></p>
        <div class="actions"><button class="primary-btn" id="tryRoleAgain">Choose another role</button></div>
      </section>`;
    $('#tryRoleAgain').addEventListener('click', () => setScreen('role'));
  }
}

function applyTeamProgress(progress, keep = {}) {
  const current = { role: keep.role || state.role, roleJoined: keep.roleJoined ?? state.roleJoined, deviceId: keep.deviceId || state.deviceId || getDeviceId() };
  state = { ...defaultState(), ...state, ...progress };
  state.role = current.role;
  state.roleJoined = current.roleJoined;
  state.deviceId = current.deviceId;
  state.route = Array.isArray(state.route) && state.route.length ? state.route : ROUTES[state.group] || [];
}

async function safeSaveGroup() {
  saveState();
  (isCaptain() ? backend('saveGroup', { group: state.group, teamName: state.teamName, route: state.route }) : Promise.resolve({ ok: true }))
    .catch(err => {
...
function renderDeviceConfirm() {
  app.innerHTML = `
    <section class="card">
      <span class="kicker">Captain device</span>
      <h2>B.O.B. has registered this as the input device.</h2>
      <p>Only this <strong>Captain / Input Device</strong> can make official submissions for the team.</p>
      <p>Buddy viewers may read and follow the same checkpoint on their own devices, but this device will be used to check in, submit responses, upload required photos, choose Eco Otter upgrades, unlock learning cards, and show the final screen to teachers.</p>
...
function renderRules() {
  app.innerHTML = `
    <section class="card">
      ...
      <ol class="info-list">
        <li>Use only the captain/input device for official submissions. Buddy viewers are read-only.</li>
...
async function refreshBuddyProgress() {
  if (!isBuddy() || !state.group) return;
  showLoading('Refreshing team progress…');
  try {
    const result = await backend('getTeamState', { group: state.group, deviceId: state.deviceId || getDeviceId() });
    if (result.teamState && result.teamState.progress) {
      applyTeamProgress(result.teamState.progress, { role: 'buddy', roleJoined: true, deviceId: state.deviceId || getDeviceId() });
      state.buddyLastRefresh = new Date().toISOString();
      saveState();
    } else {
      showToast('No captain progress has been saved yet. Try again after the captain starts.');
    }
    renderBuddyView();
  } catch (err) {
    console.error(err);
    showToast('Could not refresh. Check connection and try again.');
    renderBuddyView();
  }
}

function renderBuddyView() {
  const statusLine = state.buddyLastRefresh ? `Last refreshed: ${new Date(state.buddyLastRefresh).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}` : 'Press refresh to load captain progress.';
  app.innerHTML = `
    <section class="card">
      <span class="kicker">Buddy Viewer — Read Only</span>
      <h2>${escapeHtml(state.group || 'Team')} ${state.teamName ? '— ' + escapeHtml(state.teamName) : ''}</h2>
      <p>You can read everything and discuss with your team, but only the captain/input device can submit.</p>
      <p><small>${escapeHtml(statusLine)}</small></p>
      <div class="actions"><button class="primary-btn" id="buddyRefresh">Refresh Team Progress</button><button class="ghost-btn" id="buddyChangeTeam">Change Team</button></div>
    </section>
    ${renderBuddyCurrentContent()}`;
  $('#buddyRefresh')?.addEventListener('click', refreshBuddyProgress);
  $('#buddyChangeTeam')?.addEventListener('click', () => {
    state = defaultState();
...
function renderBuddyCurrentContent() {
  if (!state.started) {
    return `<section class="card"><h2>Waiting for captain to start</h2><p>Ask the captain to start the route, then press <strong>Refresh Team Progress</strong>.</p></section>`;
  }
  if (completedCount() >= 5 && ['finalBase', 'final', 'celebration'].includes(state.currentScreen)) {
    if (state.currentScreen === 'celebration') return renderCelebrationScreenHtmlOnly();
    if (state.currentScreen === 'final') return renderFinalScreenHtmlOnly();
    return `<section class="card"><span class="kicker">All checkpoints cleared</span><h2>Return to Teachers’ Base</h2><p>Your captain device will show the final screen and celebration reveal.</p>${renderOtterPanel('Your team has completed the route.')}</section>`;
  }
  const code = currentStationCode();
  const station = STATIONS[code];
  if (!code || !station) return `<section class="card"><h2>No current checkpoint yet.</h2><p>Press refresh after the captain starts.</p></section>`;
  const screen = state.currentScreen;
  if (screen === 'checkin') {
    return `<section class="card">${stationHeader(station, state.currentIndex, 5)}<h3>Check-in</h3><p>${escapeHtml(station.checkin.prompt)}</p><input type="text" placeholder="Captain enters this on the input device" disabled /><p><small>Read-only buddy view</small></p></section>`;
  }
  if (screen === 'missionIntro') {
    return `<section class="card">${stationHeader(station, state.currentIndex, 5)}<h3>Mission Brief</h3><div class="bob-speech">${lines(station.missionIntro)}</div><p><small>Captain starts this mission on the input device.</small></p></section>`;
  }
  if (screen === 'missionStep') {
    const step = station.steps[state.activeStep] || station.steps[0];
    return `<section class="card">${stationHeader(station, state.activeStep || 0, station.steps.length)}${renderBuddyStepBody(step)}</section>`;
  }
  if (screen === 'upgrade') return renderBuddyUpgradeContent(code);
  if (screen === 'learning') return renderBuddyLearningContent(code);
  if (screen === 'stamp') return renderBuddyStampContent();
  return `<section class="card">
    ...
    ${progressDots()}
    ${renderOtterPanel('Buddy viewer preview. Follow along with the captain device.')}
    <div class="bob-speech"><strong>B.O.B.’s clue:</strong><br>${lines(station.clue)}</div>
    <p><strong>Suggested Time:</strong><br>${station.suggestedMinutes} minutes.</p>
  </section>`;
}

function renderBuddyStepBody(step) {
  switch (step.type) {
    case 'info': return renderInfoStep(step);
    case 'checklist': return renderChecklistStep(step);
    case 'singleChoice': return renderSingleChoiceStep(step);
    case 'text': return renderTextStep(step);
    case 'textarea': return renderTextareaStep(step);
    case 'photoCaption': return renderPhotoStep(step, true);
    case 'photoOnly': return renderPhotoStep(step, false);
    case 'product': return renderProductStep(step);
    case 'lowerCarbonDay': return renderLowerCarbonDayStep(step);
    case 's5Response': return renderS5ResponseStep(step);
    default: return `<p>Unsupported step type: ${escapeHtml(step.type)}</p>`;
  }
}

function renderBuddyUpgradeContent(code) {
  const cfg = UPGRADE_SLOTS[code];
  if (!cfg) return `<section class="card"><h2>Upgrade unlocked</h2><p>Captain chooses the upgrade.</p></section>`;
  return `<section class="card"><span class="kicker">Upgrade Choice — Read Only</span><h2>${escapeHtml(cfg.label)}</h2><p>Captain chooses one of these on the input device.</p><div class="grid">${cfg.options.map(opt => `<div class="option-card"><span class="emoji">${opt.emoji}</span>${renderAccessoryCardSvg(opt.id)}<div><strong>${escapeHtml(opt.name)}</strong></div></div>`).join('')}</div>${renderOtterPanel('Buddy viewer preview.')}</section>`;
}

function renderBuddyLearningContent(code) {
  const card = LEARNING_CARDS[code];
  if (!card) return '';
  return `<section class="card"><span class="kicker">Learning Card</span><div class="learning-card"><h2>${escapeHtml(card.title)}</h2><h3>What this station teaches</h3><ol class="info-list">${card.points.map(p => `<li>${escapeHtml(p)}</li>`).join('')}</ol><div class="bob-speech"><strong>B.O.B.’s takeaway:</strong><br>${escapeHtml(card.takeaway)}</div></div></section>`;
}

function renderBuddyStampContent() {
  if (completedCount() >= 5) return `<section class="card"><span class="kicker">Final checkpoint cleared</span><h2>Final reveal still locked</h2><p>The full stamp image will appear only on the final celebration screen.</p></section>`;
  return `<section class="card"><span class="kicker">Stamp Progress</span><h2>Artwork progress</h2>${renderStampArt()}</section>`;
}

function renderFinalScreenHtmlOnly() {
  return `<section class="card"><span class="kicker">Operation Complete</span><h2>Final screen reached</h2><p>Captain device controls the final completion and celebration reveal.</p>${renderOtterPanel('Mission complete.')}</section>`;
}

function renderCelebrationScreenHtmlOnly() {
  return `<section class="card"><span class="kicker">Mission Accomplished</span><h2>Celebration reveal</h2><p>The captain device shows the full final stamp artwork.</p></section>`;
}

function lockBuddyControls() {
  $$('input, textarea, select, button').forEach(el => {
    if (el.id === 'buddyRefresh' || el.id === 'buddyChangeTeam') return;
    el.disabled = true;
    el.setAttribute('aria-disabled', 'true');
  });
  $$('[data-toggle], [data-choice], [data-need], [data-seems], [data-response], [data-upgrade]').forEach(el => {
    el.removeAttribute('data-toggle');
    el.removeAttribute('data-choice');
    el.removeAttribute('data-need');
    el.removeAttribute('data-seems');
    el.removeAttribute('data-response');
...
async function safeSaveProgress() {
  saveState();
  if (!isCaptain()) return Promise.resolve({ ok: true, readOnly: true });
...
function serializeProgress() {
  return {
    ...
    checkinUnlocked: state.checkinUnlocked,
    stationData: state.stationData,
    photos: stripPhotoPreviews(state.photos),
...
function stripPhotoPreviews(photos = {}) {
  const cleaned = {};
  Object.keys(photos || {}).forEach(code => {
    const { preview, ...rest } = photos[code] || {};
    cleaned[code] = rest;
  });
  return cleaned;
}
...
function renderStampReveal() {
  app.innerHTML = `
    <section class="card">
      ...
      ${completedCount() >= 5 ? `
        <p>Your team has completed the final checkpoint.</p>
        <p>The full collectible artwork is still locked. It will be revealed only at the final celebration screen.</p>
      ` : `
