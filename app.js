...
function mockBackend(action, payload) {
  ...
  if (action === 'validateTeacherPin') {
    return { ok: true, valid: true, mock: true };
  }
...
function renderOtter(extraUpgrade = null) {
  let upgrades = Object.values(state.upgrades || {});
  if (extraUpgrade && extraUpgrade.slot) {
    upgrades = upgrades.filter(u => u.slot !== extraUpgrade.slot);
    upgrades.push(extraUpgrade);
  }
  const upgradeHtml = upgrades.map(u => upgradeIconHtml(u, 'upgrade')).join('');
  return `
    <div class="otter" aria-label="Eco Otter preview">
      ...
      <div class="head"></div><div class="ear left"></div><div class="ear right"></div>
      <div class="face"></div><div class="eye left"></div><div class="eye right"></div><div class="nose"></div>
      <div class="whisker left one"></div><div class="whisker left two"></div><div class="whisker right one"></div><div class="whisker right two"></div>
      <div class="mouth"></div>
...
function upgradeIconHtml(u, extraClass = '') {
  const slot = escapeHtml(u.slot || '');
  const id = escapeHtml(u.id || '');
  const name = escapeHtml(u.name || 'Upgrade');
  return `<span class="${escapeHtml(extraClass)} upgrade-icon ${slot} ${id}" title="${name}" aria-label="${name}"><span class="icon-fallback">${escapeHtml(u.emoji || '')}</span></span>`;
}

function renderOtterPanel(extraUpgrade = null, message = 'Your otter collects upgrades as your team clears checkpoints.') {
  return `<div class="otter-panel">${renderOtter(extraUpgrade)}<div><strong>Eco Otter</strong><p>${escapeHtml(message)}</p></div></div>`;
}

function renderStampArt() {
  const unlocked = state.stampLayers || {};
  const op = code => unlocked[code] ? '1' : '0.08';
  const sat = code => unlocked[code] ? '1' : '0';
  const finalGlow = completedCount() >= 5 ? ' final-complete' : '';
  return `
    <div class="stamp-art${finalGlow}" aria-label="Collectible artwork progress">
      <svg class="stamp-scene" viewBox="0 0 720 430" role="img" aria-label="Funan eco-city collectible artwork">
        <defs>
          <linearGradient id="skyGrad" x1="0" x2="0" y1="0" y2="1">
            <stop offset="0" stop-color="#c9f1ff"/>
            <stop offset="0.58" stop-color="#fff7d7"/>
            <stop offset="1" stop-color="#eaffdf"/>
          </linearGradient>
          <linearGradient id="buildingGrad" x1="0" x2="1">
            <stop offset="0" stop-color="#77c8ff"/>
            <stop offset="1" stop-color="#91e6c3"/>
          </linearGradient>
          <filter id="softShadow" x="-20%" y="-20%" width="140%" height="140%">
            <feDropShadow dx="0" dy="8" stdDeviation="7" flood-color="#12303a" flood-opacity="0.18"/>
          </filter>
          <filter id="muted"><feColorMatrix type="saturate" values="0"/></filter>
        </defs>
        <rect width="720" height="430" rx="34" fill="url(#skyGrad)"/>
        <circle cx="610" cy="72" r="44" fill="#ffd166" opacity="0.95"/>
        <path d="M0 355 C110 324 165 362 248 335 C344 303 401 365 503 335 C610 303 660 330 720 313 L720 430 L0 430 Z" fill="#caefc6" opacity="0.72"/>
        <g class="stamp-scene-layer layer-s4" style="--layer-opacity:${op('S4')};--layer-sat:${sat('S4')}">
          <g filter="url(#softShadow)">
            <path d="M272 352 L272 142 L360 79 L452 142 L452 352 Z" fill="url(#buildingGrad)"/>
            <path d="M291 352 L291 168 L360 118 L433 168 L433 352 Z" fill="#e8fbff" opacity="0.62"/>
            <g fill="#ffffff" opacity="0.86">
              <rect x="311" y="178" width="32" height="23" rx="6"/><rect x="377" y="178" width="32" height="23" rx="6"/>
              <rect x="311" y="222" width="32" height="23" rx="6"/><rect x="377" y="222" width="32" height="23" rx="6"/>
              <rect x="311" y="266" width="32" height="23" rx="6"/><rect x="377" y="266" width="32" height="23" rx="6"/>
            </g>
            <rect x="345" y="300" width="36" height="52" rx="10" fill="#4dabf7" opacity="0.85"/>
          </g>
          <path d="M188 357 C230 329 284 324 338 346 C397 371 442 354 492 328 C540 303 594 309 642 335" fill="none" stroke="#087f8c" stroke-width="12" stroke-linecap="round" stroke-dasharray="1 22" opacity="0.45"/>
          <text x="505" y="245" class="stamp-word">lyf</text>
          <rect x="516" y="260" width="76" height="42" rx="15" fill="#fff" opacity="0.88"/>
          <text x="529" y="288" class="stamp-small">MRT</text>
        </g>
        <g class="stamp-scene-layer layer-s1" style="--layer-opacity:${op('S1')};--layer-sat:${sat('S1')}">
          <path d="M247 145 C283 107 334 103 369 128 C407 99 466 121 475 171 C442 179 414 173 391 153 C356 178 304 179 247 145 Z" fill="#66d26f" filter="url(#softShadow)"/>
          <path d="M286 146 C310 128 335 130 353 150" fill="none" stroke="#1b9c59" stroke-width="6" stroke-linecap="round"/>
          <path d="M410 151 C431 138 448 142 461 156" fill="none" stroke="#1b9c59" stroke-width="6" stroke-linecap="round"/>
          <g fill="#ffd166">
            <circle cx="302" cy="126" r="8"/><circle cx="426" cy="133" r="7"/><circle cx="367" cy="119" r="6"/>
          </g>
          <rect x="321" y="151" width="104" height="20" rx="10" fill="#fff" opacity="0.75"/>
          <path d="M327 161 H415" stroke="#087f8c" stroke-width="5" stroke-linecap="round"/>
        </g>
        <g class="stamp-scene-layer layer-s2" style="--layer-opacity:${op('S2')};--layer-sat:${sat('S2')}">
          <g filter="url(#softShadow)">
            <rect x="65" y="237" width="137" height="98" rx="24" fill="#fff5d6"/>
            <path d="M65 266 Q133 218 202 266" fill="#ffb86b"/>
            <rect x="91" y="279" width="35" height="38" rx="7" fill="#88d498"/>
            <rect x="139" y="279" width="39" height="38" rx="7" fill="#95d5ff"/>
          </g>
          <path d="M106 288 Q107 259 132 259 Q158 259 159 288" fill="none" stroke="#087f8c" stroke-width="7" stroke-linecap="round"/>
          <circle cx="181" cy="238" r="18" fill="#b7eb63"/>
          <path d="M177 238 l6 6 l12 -15" fill="none" stroke="#087f8c" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/>
        </g>
        <g class="stamp-scene-layer layer-s3" style="--layer-opacity:${op('S3')};--layer-sat:${sat('S3')}">
          <path d="M35 381 C132 336 234 376 318 347 C426 309 500 363 685 316" fill="none" stroke="#087f8c" stroke-width="20" stroke-linecap="round" opacity="0.82"/>
          <path d="M35 381 C132 336 234 376 318 347 C426 309 500 363 685 316" fill="none" stroke="#fff" stroke-width="5" stroke-linecap="round" stroke-dasharray="22 22" opacity="0.95"/>
          <g transform="translate(497 295)" filter="url(#softShadow)">
            <circle cx="0" cy="34" r="22" fill="none" stroke="#12303a" stroke-width="7"/>
            <circle cx="72" cy="34" r="22" fill="none" stroke="#12303a" stroke-width="7"/>
            <path d="M0 34 L31 4 L48 34 L19 34 L45 34 L72 34" fill="none" stroke="#12303a" stroke-width="7" stroke-linecap="round" stroke-linejoin="round"/>
            <circle cx="31" cy="4" r="7" fill="#ff6b6b"/>
          </g>
          <rect x="576" y="246" width="74" height="46" rx="14" fill="#ffffff" opacity="0.88"/>
          <path d="M596 274 h24 m-9 -15 l9 15 l-9 15" stroke="#2fbf71" stroke-width="7" fill="none" stroke-linecap="round" stroke-linejoin="round"/>
        </g>
        <g class="stamp-scene-layer layer-s5" style="--layer-opacity:${op('S5')};--layer-sat:${sat('S5')}">
          <g filter="url(#softShadow)">
            <rect x="530" y="325" width="41" height="62" rx="14" fill="#4dabf7"/>
            <rect x="540" y="313" width="21" height="19" rx="5" fill="#087f8c"/>
            <rect x="593" y="332" width="61" height="43" rx="18" fill="#ffd166"/>
            <circle cx="641" cy="331" r="12" fill="#ff9f5a"/>
          </g>
          <path d="M613 352 q16 13 34 0" fill="none" stroke="#12303a" stroke-width="5" stroke-linecap="round"/>
          <circle cx="579" cy="385" r="20" fill="#b7eb63"/>
          <path d="M570 386 l8 8 l15 -19" stroke="#087f8c" stroke-width="5" fill="none" stroke-linecap="round" stroke-linejoin="round"/>
        </g>
        <g class="stamp-scene-layer stamp-otter" style="--layer-opacity:${completedCount() >= 5 ? '1' : '0.16'};--layer-sat:${completedCount() >= 5 ? '1' : '0'}">
          <ellipse cx="250" cy="365" rx="57" ry="25" fill="#8b5e3c" opacity="0.25"/>
          <path d="M200 350 C178 333 183 304 215 318 C243 331 240 360 217 368 Z" fill="#8a5938"/>
          <ellipse cx="255" cy="315" rx="45" ry="66" fill="#9a6743"/>
          <ellipse cx="255" cy="329" rx="27" ry="42" fill="#f7ddb4"/>
          <circle cx="255" cy="250" r="39" fill="#9a6743"/>
          <circle cx="228" cy="231" r="13" fill="#875634"/><circle cx="282" cy="231" r="13" fill="#875634"/>
          <ellipse cx="255" cy="266" rx="24" ry="17" fill="#f7ddb4"/>
          <circle cx="243" cy="252" r="5" fill="#12303a"/><circle cx="267" cy="252" r="5" fill="#12303a"/>
          <circle cx="255" cy="262" r="6" fill="#12303a"/>
          <path d="M249 274 q6 7 13 0" fill="none" stroke="#12303a" stroke-width="4" stroke-linecap="round"/>
        </g>
      </svg>
      <div class="stamp-caption">${completedCount() >= 5 ? 'Final collectible scene complete.' : 'Muted parts will become colourful as your team clears checkpoints.'}</div>
    </div>`;
}
...
function renderPhotoStep(step, withCaption) {
  ...
  return `
    ...
    <div class="file-input">
      ...
      ${photo ? (photo.bypassed ? `<span class="badge warning">Photo bypassed by teacher</span>` : `<span class="badge ok">Photo uploaded: ${escapeHtml(photo.fileName)}</span>${photo.preview ? `<img src="${photo.preview}" class="file-preview" alt="Uploaded photo preview" />` : ''}`) : '<span class="badge warning">Photo required for this checkpoint</span>'}
    </div>
...
function showUploadFailureModal(code, message) {
  ...
  showModal(`
    ...
    <div class="actions"><button class="primary-btn" id="retryUpload">Retry Upload</button><button class="secondary-btn" id="copyHelp">Copy Help Message</button><button class="secondary-btn" id="teacherBypassUpload">Teacher Bypass</button><button class="ghost-btn" id="closeModal">Close</button></div>`);
  ...
  $('#teacherBypassUpload').addEventListener('click', () => showPhotoBypassModal(code, message));
...
function showPhotoBypassModal(code, failureMessage = '') {
  ...
  showModal(`
    ...
    <input id="bypassNote" type="text" placeholder="e.g. Urban Farm rooftop" />
...
async function handlePhotoBypass(code, failureMessage = '') {
  ...
  try {
    await backend('validateTeacherPin', { teacherPin: pin });
    localStorage.setItem(CONFIG.TEACHER_PIN_LOCAL_KEY, pin);
    const step = STATIONS[code].steps[state.activeStep];
    const attempt = (state.photos[code]?.attempt || 0) + 1;
    const bypassRecord = {
    ...
    showToast('Bypass failed. Check teacher PIN or backend.');
  }
}
...
function renderUpgrade() {
  ...
  const selectedOption = cfg.options.find(o => o.id === chosen);
  const previewUpgrade = selectedOption ? { ...selectedOption, slot: cfg.slot } : null;
  app.innerHTML = `
    <section class="card">
      ...
      <div id="upgradePreview">${renderOtterPanel(previewUpgrade, selectedOption ? `${selectedOption.name} preview selected.` : 'Choose an upgrade to preview it on your otter.')}</div>
      <h3>Choose one:</h3>
      <div class="grid two">
        ${cfg.options.map(opt => `<button type="button" class="option-card ${chosen === opt.id ? 'selected' : ''}" data-upgrade="${escapeHtml(opt.id)}">${upgradeIconHtml({ ...opt, slot: cfg.slot }, 'option-upgrade')}<div><strong>${escapeHtml(opt.name)}</strong><span>Tap to preview this look.</span></div></button>`).join('')}
      </div>
  ...
  $$('[data-upgrade]').forEach(btn => btn.addEventListener('click', () => {
    ...
    $('#upgradePreview').innerHTML = renderOtterPanel({ ...option, slot: cfg.slot }, `${option.name} preview selected.`);
  }));
  $('#confirmUpgrade').addEventListener('click', async () => {
    ...
    saveState();
    await safeSaveProgress();
    renderUpgradeApplied(code, option, cfg);
  });
}

function renderUpgradeApplied(code, option, cfg) {
  app.innerHTML = `
    <section class="card">
      <span class="kicker">Upgrade applied!</span>
      <h2>${escapeHtml(option.name)} added.</h2>
      ${renderOtterPanel({ ...option, slot: cfg.slot }, 'B.O.B. approves this look. Your otter is one step closer to its final form.')}
      <div class="actions"><button class="primary-btn" id="continueAfterUpgrade">Continue</button></div>
    </section>`;
  $('#continueAfterUpgrade').addEventListener('click', () => setScreen('learning'));
}
