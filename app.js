...
function renderOtterAccessoryHtml(id) {
  const asset = OTTER_ASSET_MAP[id];
  if (!asset) return '';
  return `<img src="${asset.src}" class="otter-accessory ${asset.className}" alt="" aria-hidden="true">`;
}

function renderOtter() {
  const selected = Object.values(state.upgrades || {}).filter(Boolean).map(item => item.id);
  const backMarkup = selected.filter(id => OTTER_ASSET_MAP[id]?.z === 'back').map(renderOtterAccessoryHtml).join('');
  const frontMarkup = selected.filter(id => OTTER_ASSET_MAP[id]?.z !== 'back').map(renderOtterAccessoryHtml).join('');
  return `
...
async function drawOtterToCanvas(canvas) {
  const ctx = canvas.getContext('2d');
  const { width, height, base, shadow, slots } = OTTER_CANVAS;
  canvas.width = width;
  ...
  ctx.beginPath();
  ctx.ellipse(shadow.x + shadow.w / 2, shadow.y + shadow.h / 2, shadow.w / 2, shadow.h / 2, 0, 0, Math.PI * 2);
  ctx.fill();

  const chosen = Object.values(state.upgrades || {}).filter(Boolean).map(item => item.id);
  const baseImg = await loadImageAsset(base.src);

  for (const id of chosen.filter(id => OTTER_ASSET_MAP[id]?.z === 'back')) {
    const slot = slots[id];
    if (!slot) continue;
    const img = await loadImageAsset(OTTER_ASSET_MAP[id].src);
...
async function downloadFinalAvatar() {
  ...
  try {
    ...
    await drawOtterToCanvas(canvas);
    canvas.toBlob(blob => {
      if (!blob) {
